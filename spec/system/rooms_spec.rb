...
RSpec.describe 'チャットルームの削除機能', type: :system do
  ...
  it 'チャットルームを削除すると、関連するメッセージがすべて削除されている' do
    ...
    # メッセージ情報を5つDBに追加する
      ## 複数DBに保存するには、create_listメソッドを使用します。
      ## 第１引数には作成したいインスタンスをシンボル型で記述し、第２引数に作成するレコードの個数を指定します。
      ## また、第３引数以降では、オプションとして「カラムの値を指定」することもできます。外部キーの値を指定するとhas_many関係のデータを生成することもできます。
    ...
    expect {
      ## find_linkメソッドで「'チャットを終了する',href:room_path(@room_user.room)」を取得して、クリックします。
